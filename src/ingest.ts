...
export function latestStarredAt(db: DB): number | null {
  const row = db.prepare<{ t: number | null }, []>('SELECT max(starred_at) AS t FROM star').get()
  return row?.t ?? null
}
...
export function ingestPage(db: DB, syncId: number, items: StarItem[]): PageResult {
  const find = db.prepare<{ sha256: string; unstarred_sync: number | null }, [number]>(
    'SELECT sha256, unstarred_sync FROM star WHERE repo_id = ?',
  )
  ...
  const run = db.transaction((): PageResult => {
    ...
    ).run(r.seen, r.added, r.changed, syncId)
...
export function finishSync(db: DB, syncId: number, complete: boolean): { gone: number } {
  const run = db.transaction((): { gone: number } => {
    const gone = complete
      ? Number(
          db
